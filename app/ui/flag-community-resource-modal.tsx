...
export function FlagCommunityResourceModal({ resourceId, onClose, onFlagged }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/community-resources/${resourceId}/flag`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ reason }),
      })
      const json = await res.json()
      if (res.status === 409) { setError('You have already reported this resource.'); setSub(false); return }
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm rounded-2xl p-6 shadow-2xl"
        ...
        <h2 className="mb-2 text-lg font-bold" style={{ color: '#f5f0eb' }}>Report Resource</h2>
        <p className="mb-4 text-sm" style={{ color: '#9a8e87' }}>
          Let us know why this resource should be reviewed. Resources reported 5 times are hidden automatically.
        </p>
