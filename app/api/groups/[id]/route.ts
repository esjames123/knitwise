...
export async function PUT(
  ...
  for (const f of ['description', 'what_to_bring', 'community_outreach',
  ...
  if (error) {
    console.error('[PUT /api/groups/id]', error.message)
...
export async function DELETE(
  ...
  if (error) {
    console.error('[DELETE /api/groups/id]', error.message)
    return Response.json({ error: error.message }, { status: 500 })
  }
  if (count === 0) {
    return Response.json({ error: 'Not found or not authorized' }, { status: 404 })
